 ...
 * Handles fetching Pre-Forming QC data when SO No is entered and populates form fields
 */
...
(() => {
  ...
  /**
   * Fetch Pre-Forming QC data by SO No
   */
  async function fetchPreFormingQCData(soNo) {
  ...
  async function handleSearchClick() {
    ...
    try {
      ...
      const data = await fetchPreFormingQCData(soNo);
  ...
  async function handleFormSubmit(event) {
    ...
    try {
      ...
      const payload = {
        ...
        pre_forming_qc_done_by: empId.toString(), // Convert to string
      };

      // API endpoint
      const API_ENDPOINT = '/api/v2/pre-forming-qc-update/';
      ...
      if (preFormingQCAvailableQuantityInput) {
        preFormingQCAvailableQuantityInput.value = '';
      }
