 ...
 * Handles fetching Leaded QC data when SO No is entered and populates form fields
 */
...
(() => {
  ...
  /**
   * Fetch Leaded QC data by SO No
   */
  async function fetchLeadedQCData(soNo) {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        const errorMessage = errorData.error || errorData.message || 'Failed to fetch Leaded QC data';
      ...
      console.error('Leaded QC data fetch failed:', error);
      // Only show error if it's not a 404 (not found)
      if (!error.message.includes('No entry found')) {
        showToast(error.message || 'Unable to fetch Leaded QC data. Please try again.', 'error');
      }
      return null;
    }
  }
  ...
  function updateSubmitButtonState() {
    const form = document.getElementById('leadedQCForm');
    const submitButton = form ? form.querySelector('button[type="submit"]') : null;
    const leadedQCAvailableQuantityInput = document.getElementById('leadedQCAvailableQuantity');
    
    if (!submitButton || !leadedQCAvailableQuantityInput) {
      return;
    }

    const availableQuantity = parseInt(leadedQCAvailableQuantityInput.value, 10) || 0;
  ...
  function populateFormFields(data) {
    ...
    // Populate Leaded QC Available Quantity
    const leadedQCAvailableQuantityInput = document.getElementById('leadedQCAvailableQuantity');
    if (leadedQCAvailableQuantityInput && data.leaded_qc_available_quantity) {
      leadedQCAvailableQuantityInput.value = data.leaded_qc_available_quantity;
    }
  ...
  async function handleSearchClick() {
    ...
    const searchBtn = document.getElementById('searchLeadedQCBtn');
    ...
    try {
      ...
      if (leadedQCAvailableQuantityInput) {
        leadedQCAvailableQuantityInput.value = 'Loading...';
        leadedQCAvailableQuantityInput.style.opacity = '0.6';
      }

      const data = await fetchLeadedQCData(soNo);
      ...
      if (leadedQCAvailableQuantityInput) {
        leadedQCAvailableQuantityInput.style.opacity = '1';
      }

      if (data) {
        populateFormFields(data);
        showToast('Leaded QC data loaded successfully', 'success');
      } else {
  ...
  async function handleFormSubmit(event) {
    ...
    try {
      ...
      // Get current Leaded QC available quantity
      const currentLeadedQCQuantity = leadedQCAvailableQuantityInput ? 
        (parseInt(leadedQCAvailableQuantityInput.value, 10) || 0) : 0;

      // Validate forwarding quantity doesn't exceed available
      if (forwardingQuantity > currentLeadedQCQuantity) {
        throw new Error(`Forwarding quantity (${forwardingQuantity}) cannot be greater than available quantity (${currentLeadedQCQuantity})`);
      }
      ...
      if (!response.ok) {
        const errorData = await response.json().catch(() => ({}));
        throw new Error(errorData.error || errorData.message || 'Failed to update Leaded QC data');
      }
      ...
      console.error('Leaded QC form submission failed:', error);
      showToast(error.message || 'Unable to update Leaded QC data. Please try again.', 'error');
    } finally {
  ...
  /**
   * Initialize Leaded QC form handler
   */
  function init() {
    ...
    if (form) {
      ...
      // Listen for changes in Leaded QC Available Quantity to update submit button state
      const leadedQCAvailableQuantityInput = document.getElementById('leadedQCAvailableQuantity');
      if (leadedQCAvailableQuantityInput) {
        leadedQCAvailableQuantityInput.addEventListener('input', updateSubmitButtonState);
        leadedQCAvailableQuantityInput.addEventListener('change', updateSubmitButtonState);
      }
      
      // Initial state check
      updateSubmitButtonState();
    } else {
      console.warn('Leaded QC form not found');
    }

    console.log('Leaded QC API handler initialized');
  }
