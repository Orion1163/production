/**
 * Cleaning Dynamic Fields Handler
...
(() => {
  ...
  const CLEANING_SERIAL_SEARCH_URL = '/api/v2/cleaning-serial-number-search/';
  const CLEANING_SUBMIT_URL = '/api/v2/cleaning-submit/';
  ...
  async function searchSerialNumber(serialNumber, usidInput) {
    ...
    try {
      ...
      const response = await fetch(`${CLEANING_SERIAL_SEARCH_URL}?${params.toString()}`, {
        method: 'GET',
        headers: {
          'Content-Type': 'application/json',
        },
  ...
  function addFieldRow() {
    ...
    addButtons.forEach(btn => {
      ...
      `;
      btn.setAttribute('onclick', 'window.cleaningAPI.removeFieldRow(this)');
    ...
    fieldRow.innerHTML = `
      ...
      <button type="button" class="add-btn" id="addBtn_${rowIndex}" onclick="window.cleaningAPI.addFieldRow()" title="Add another row">
      ...
      setTimeout(() => {
        ...
        if (remainingRows.length > 0) {
          ...
          if (lastRowButton) {
            ...
            `;
            lastRowButton.setAttribute('onclick', 'window.cleaningAPI.addFieldRow()');
  ...
  /**
   * Initialize Cleaning form handler
   */
  function init() {
    ...
    const checkbox = document.getElementById('cleaningCheckbox');
    ...
    console.log('Cleaning API handler initialized');
  }
  ...
  async function handleFormSubmit(event) {
    ...
    try {
      ...
      // Check if Cleaning checkbox is checked
      const checkbox = document.getElementById('cleaningCheckbox');
      if (!checkbox || !checkbox.checked) {
        throw new Error('Please check the Cleaning checkbox to submit data');
      }
      ...
      const response = await fetch(CLEANING_SUBMIT_URL, {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json',
      ...
      if (!response.ok) {
        ...
        throw new Error(errorData.error || errorData.message || 'Failed to submit Cleaning data');
      }

      const result = await response.json();
      
      // Show success message
      let successMessage = result.message || 'Cleaning data submitted successfully!';
      ...
      // Reset checkbox
      const checkboxReset = document.getElementById('cleaningCheckbox');
      const checkboxCard = document.getElementById('checkboxCard');
      if (checkboxReset) {
      ...
      if (fieldsContainer) {
        ...
        if (firstRow) {
          ...
          if (firstButton) {
            ...
            `;
            firstButton.setAttribute('onclick', 'window.cleaningAPI.addFieldRow()');
      ...
      console.error('Cleaning form submission failed:', error);
      showToast(error.message || 'Unable to submit Cleaning data. Please try again.', 'error');
    } finally {
