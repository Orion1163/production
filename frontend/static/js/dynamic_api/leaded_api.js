 ...
 * Fetches Leaded procedure config (custom_fields, custom_checkboxes), renders dynamic checkboxes,
 * fetches Leaded data by Kit No, and submits update including custom data.
 */
...
(() => {
  ...
  const LEADED_UPDATE_URL = '/api/v2/leaded-update/';
  const LEADED_CONFIG_URL = '/api/v2/leaded-procedure-config/';
  ...
  async function fetchLeadedConfig() {
    const partNo = getPartNo();
    if (!partNo) return null;
    try {
      const response = await fetch(`${LEADED_CONFIG_URL}${partNo}/`, {
        method: 'GET',
      ...
      console.error('Error fetching Leaded config:', error);
      return null;
    }
  }
  ...
  function createCheckboxField(checkboxConfig, index) {
    ...
    if (checkboxName.toLowerCase() === 'leaded') return null;
  ...
  function renderDynamicCheckboxes(config) {
    const checkboxesSection = document.getElementById('leadedCheckboxesSection');
    const checkboxesGrid = document.getElementById('leadedCheckboxesGrid');
    ...
    const filtered = customCheckboxes.filter(cb => (cb.name || '').toLowerCase() !== 'leaded');
  ...
  async function handleFormSubmit(event) {
    ...
    const leadedCheckboxesSection = document.getElementById('leadedCheckboxesSection');
    const leadedCheckboxes = document.querySelectorAll('#leadedCheckboxesGrid .custom-checkbox');
    if (leadedCheckboxesSection && leadedCheckboxesSection.style.display !== 'none' && leadedCheckboxes.length > 0) {
      const allChecked = Array.prototype.every.call(leadedCheckboxes, function (cb) { return cb.checked; });
      if (!allChecked) {
    ...
    try {
      ...
      const leadedForm = document.getElementById('leadedForm');
      if (leadedForm) {
        leadedForm.querySelectorAll('input[type="text"], input[type="number"]').forEach(function (input) {
          const name = input.name || input.id;
          if (name && !['kitNo', 'soNo', 'leadedAvailableQuantity', 'forwardingQuantity'].includes(name)) {
            const val = input.value != null ? input.value.trim() : '';
            if (name) customFields[name] = val;
          }
        });
        leadedForm.querySelectorAll('input[type="checkbox"]').forEach(function (checkbox) {
      ...
      const payload = {
        ...
        leaded_done_by: empId.toString(),
        custom_fields: customFields,
        custom_checkboxes: customCheckboxes,
      };

      const response = await fetch(LEADED_UPDATE_URL, {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json',
          'Accept': 'application/json',
      ...
      if (soNoInput) soNoInput.value = '';
      if (leadedAvailableQuantityInput) leadedAvailableQuantityInput.value = '';
      document.querySelectorAll('#leadedCheckboxesGrid .custom-checkbox').forEach(function (cb) {
        cb.checked = false;
  ...
  function init() {
    ...
    const partNo = getPartNo();
    if (partNo) {
      fetchLeadedConfig().then(function (config) {
