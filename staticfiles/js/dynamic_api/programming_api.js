/**
 * Programming Dynamic Fields Handler
...
(() => {
  ...
  const PROGRAMMING_SERIAL_SEARCH_URL = '/api/v2/programming-serial-number-search/';
  const PROGRAMMING_SUBMIT_URL = '/api/v2/programming-submit/';
  ...
  async function searchSerialNumber(serialNumber, usidInput) {
    ...
    try {
      ...
      const response = await fetch(`${PROGRAMMING_SERIAL_SEARCH_URL}?${params.toString()}`, {
        method: 'GET',
        headers: {
          'Content-Type': 'application/json',
        },
  ...
  function addFieldRow() {
    ...
    addButtons.forEach(btn => {
      ...
      `;
      btn.setAttribute('onclick', 'window.programmingAPI.removeFieldRow(this)');
    ...
    fieldRow.innerHTML = `
      ...
      <button type="button" class="add-btn" id="addBtn_${rowIndex}" onclick="window.programmingAPI.addFieldRow()" title="Add another row">
      ...
      setTimeout(() => {
        ...
        if (remainingRows.length > 0) {
          ...
          if (lastRowButton) {
            ...
            `;
            lastRowButton.setAttribute('onclick', 'window.programmingAPI.addFieldRow()');
  ...
  /**
   * Initialize Programming form handler
   */
  function init() {
    ...
    console.log('Programming API handler initialized');
  }
  ...
  async function handleFormSubmit(event) {
    ...
    try {
      ...
      console.log('Programming Payload:', payload);

      // Get CSRF token
      const csrfToken = getCookie('csrftoken');

      // Submit to API (PUT request to update existing entries)
      const response = await fetch(PROGRAMMING_SUBMIT_URL, {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json',
      ...
      if (!response.ok) {
        ...
        throw new Error(errorData.error || errorData.message || 'Failed to submit Programming data');
      }

      const result = await response.json();
      
      // Show success message
      let successMessage = result.message || 'Programming data submitted successfully!';
      ...
      if (fieldsContainer) {
        ...
        if (firstRow) {
          ...
          if (firstButton) {
            ...
            `;
            firstButton.setAttribute('onclick', 'window.programmingAPI.addFieldRow()');
      ...
      console.error('Programming form submission failed:', error);
      showToast(error.message || 'Unable to submit Programming data. Please try again.', 'error');
    } finally {
